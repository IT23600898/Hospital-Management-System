...
<head>
  ...
  <title>HospitaLink | Edit Patient</title>
...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen flex">
  ...
  <div class="flex-1 flex flex-col">
    ...
    <main class="flex-1 p-6 space-y-6">
      ...
      <!-- Full Width Form -->
      <form action="${pageContext.request.contextPath}/patient" method="POST" onsubmit="return validateForm()" class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 w-full">
        ...
        <div class="mt-6">
          ...
          <textarea id="address" name="address" rows="3" required
            class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none">${patient.address}</textarea>
        </div>
  ...
  document.querySelector("form").addEventListener("submit", function(e) {
    ...
    if (!fullName || !birthDate || birthDate > today || !gender || !contactNumber || !address) {
    ...
    if (!/^\d{9,15}$/.test(contactNumber)) {
      alert("Contact number must be 9 to 15 digits.");
      e.preventDefault();
      return;
    }
  });
